...
| [x] | Logging levels and redaction | — | `authorization`, `proxy-authorization`, `x-api-key`, `api-key`, `cookie`, `set-cookie` + names containing `token`/`secret` | credential headers with suffix kept | levels follow upstream: one line per attempt (`<- status in ms (request id)`) and per retry (`retrying in Xms (retry n/total) after <class status=…>`), transport failures, timeouts and caller aborts at **INFO**; request/response headers and bodies at **DEBUG** (= both SDKs); dropped answer kinds at **WARNING** (= Python). Default gate **WARNING** (= JS's `warn`), so nothing is emitted for healthy calls unless opted in via `TYPESAFE_LOG_LEVEL` or the builder. Sink is `System.Logger`. Retry lines never carry body text (body-derived messages are DEBUG); full redaction of Python's list plus `token`/`secret` names on request and response headers; `ClientConfig.toString()` omits the key and redacts default headers | config | `HttpJevClientTest.loggingHonoursTheClientLevelAndRedactsCredentials` (captures JUL records at OFF/INFO/DEBUG/TRACE), `debugRetryLogsCarryNoBodyText`, `configToStringRedactsCredentials` |
